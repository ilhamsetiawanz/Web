    ...
    <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
      <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
          <h3 class="text-center p-5 font-bold">Edit Data Penyakit</h3>
          <form action="{{ route('edit-penyakit', $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                <button type="submit" class="inline-flex w-full justify-center rounded-md px-2 py-1 text-sm font-semibold border border-teal-500 text-teal-500 hover:border-teal-500 hover:text-white hover:bg-teal-500 sm:ml-3 sm:w-auto">Edit Penyakit</button>
